...
res.a <- tclust2(x, k = 3, alpha=0.1, restr.fact = 1, restr= "eigen",
                 equal.weights = TRUE)

## applying tclust with restr.fact = 50, restr= "eigen"
res.b <- tclust2(x, k = 3, alpha=0.1, restr.fact = 50, restr= "eigen",
                 equal.weights = FALSE)

## applying tclust with restr.fact = 1, restr= "deter"
res.c <- tclust2(x, k = 3, alpha=0.1, restr.fact = 1, restr= "deter",
                 equal.weights = TRUE)

## applying tclust with restr.fact = 50, restr= "deter"
res.d <- tclust2(x, k = 3, alpha=0.1, restr.fact = 50, restr= "deter",
                 equal.weights = TRUE)
...
x <- rbind(MASS::mvrnorm(200, c (0, 0), diag (2)),
  ...
  ## applying tclust
clus.4 <- tclust2(x, k = 3, alpha = 0.0, restr.fact = 1)
...
clus <- tkmeans2(geyser2.mod, k = 3, alpha = 0.03)
...
title ("(d) Non-elliptical t components", line = 1.6)
 ...
 ## applying tclust without trimming
clus <- tclust2(rnd.2, k = 3, alpha = 0.0)
plot(clus, xlim = c (-10, 20), ylim = ylim, xlab = xlab, ylab = ylab,
      main.pre = "(e)")

 ## applying tclust with trimming (5%)
clus <- tclust2(rnd.2, k = 3, alpha = 0.05)
plot(clus, xlim = c (-10, 20), ylim = ylim, xlab = xlab, ylab = ylab,
      main.pre = "(f)")
                                     ...
                                     size = sqrt (qchisq (0.95, 2)), ...)
{
  ...
  for (i in 1:k)
    robClus:::.doEllipses(cov = clus$parameters$variance$sigma[,, i],
...
mixt <- rbind(MASS::mvrnorm(360, c (  0,  0), matrix (c (1,  0,  0,  1), ncol = 2)),
  ...
  ## applying tclust
set.seed (100)
clus.1 <- tclust2(mixt, k = 3, alpha =  0.0, restr.fact = 50)
clus.2 <- tclust2(mixt, k = 2, alpha = 0.05, restr.fact =  8)
...
################
  ...
  ## computing ctlcurves
ctl <- ctlcurves2(mixt, restr.fact = 50, alpha = seq (0, 0.2, by = 0.05))
...
geyser1 <- geyser2[, 1, drop = FALSE]
  ## applying tkmeans
plot(tkmeans2(geyser1, k = 2, alpha = 0.03), jitter = TRUE, tol.lwd = 2,
      main.pre = "(a)")

  ## adding a random dimension to geyser2
geyser3 <- cbind(geyser2, rnorm (nrow (geyser2)))
  ## applying tkmeans
plot(tkmeans2(geyser3, k = 3, alpha = 0.03), main.pre = "(b)")
...
mixt2 <- rbind (
  ...
  ## applying tclust
set.seed (100)
clus.w <- tclust2(mixt2, k = 3, alpha = 0.1, restr.fact = 1,
                  equal.weights = TRUE)
...
robClus:::plot_DiscrFact_p2(discr.clus.w, xlim = c (-70, 0), main.pre = "(b)")
robClus:::plot_DiscrFact_p3(discr.clus.w, tol.lty = 2, main.pre = "(c)")
...
data (swissbank)
set.seed (0)
fig6.ctl <- ctlcurves2(swissbank, k = 1:4, alpha = seq (0, 0.3, by = 0.025),
...
fig7.clus <- tclust2(swissbank, k = 2, alpha = .1, restr.fact = 50)
...
clus <- tclust2(swissbank, k = 2, alpha=.1, restr.fact=50)
